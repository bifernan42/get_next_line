...
/*   Updated: 2024/02/01 17:20:39 by bifernan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	b2[1024][BUFFER_SIZE];
	t_val		s_v;
	...
	s_v.b2_size = ft_b2handler(b2[fd], s_v.b1, &s_v.line, &s_v);
	if (s_v.b2_size == -1)
		return (NULL);
	if (s_v.line != NULL)
		return (s_v.line);
	...
	s_v.reaturn = read(fd, s_v.b1, BUFFER_SIZE);
	s_v.ni = ft_findindex(s_v.b1, BUFFER_SIZE);
	if (ft_get_list(fd, b2[fd], &s_v) == -1)
		return (ft_dlstclear(s_v.dlst), NULL);
	if (ft_get_line(b2[fd], &s_v) == -1)
		return (NULL);
	ft_dlstclear(s_v.dlst);
	return (s_v.line);
}
